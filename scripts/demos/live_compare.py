"""Live A/B/C 3-way 영상 비교 뷰어 with Controls.

좌: 1× MoodEQ (spec) / 중: 3× Demo (exaggerated) / 우: Original
하단 3개 실시간 스펙트럼 + 우측 컨트롤 패널
...
PANEL_COLORS = {
    "proc": "#d62728",   # 1× MoodEQ
    "demo": "#9467bd",   # 3× Demo
    "orig": "#1f3a68",   # Original
}
PANEL_LABELS = {"proc": "1× MoodEQ", "demo": "3× Demo", "orig": "Original"}

AUDIO_LABELS = ["1× MoodEQ", "3× Demo", "Original", "Mute"]
...
def main() -> int:
    ...
    p.add_argument("--processed", default="runs/demo_kakao/kakao_eq_applied.mp4",
                   help="1× MoodEQ (spec) 영상")
    p.add_argument("--demo", default="runs/demo_kakao/kakao_eq_demo3x.mp4",
                   help="3× demo (exaggerated) 영상")
    ...
    EQ_Y_RANGE = (-8.0, 8.0)
    EQ_GAIN_MULT = {"proc": 1.0, "demo": 3.0, "orig": 0.0}
